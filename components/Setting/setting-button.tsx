...
export default function Setting({ active }: AppProps) {
  ...
  return (
    <>
      ...
      >
        <div
          className={`z-10 overflow-hidden bg-${setting.theme ? "zinc-300" : "gray-700"} p-2 w-[40px] h-[40px] -translate-x-2 -translate-y-2 rounded-full absolute transition-all duration-300 ease-in-out ${
            isHover || offCanvas ? "mask-hover" : "mask"
          }`}
        >
          <div className="absolute">
            <FontAwesomeIcon className={`translate-x-[1px] px-[2px] text-center ${setting.theme ? "text-pri-color-dark" : "text-white"}`} icon={faGear} size={"lg"} />
          </div>
        </div>
        <FontAwesomeIcon className={`translate-x-[1px] px-[2px] text-center ${setting.theme ? "text-white" : "text-gray-700"}`} icon={faGear} size={"lg"} />
      </div>
      <CSSTransition nodeRef={ref} in={offCanvas && active} unmountOnExit timeout={300} classNames={"off-canvas"}>
